...
class FreezeThawGP(BaseModel):
    ...
    def __init__(self,
        ...
        if sampleSet is not None:
            self.ps = PredSamples(samples=sampleSet, x_train=self.X, y_train=self.y, x_test=self.X,
                                  predHyper=True, predOld=True, predNew=True, invChol=True, samenoise=True)
    ...
    def train(self, X=None, Y=None, do_optimize=True):
        ...
        if do_optimize:
            lik = LikIntegrate(y_train=self.y, invChol=True,
                               horse=True, samenoise=True)
            sampleSet = lik.create_configs(
                x_train=self.X, y_train=self.y, hyper_configs=12, chain_length=100, burnin_steps=100)
    ...
    def predictive_new(self, configuration, steps=1):
        """
        Predicts mean and std2 for completely new configuration X. The prediction is averaged for
        all GP hyperparameter samples. They are integrated out.
        ...
        mean, std2 = self.ps.pred_new(
            steps=steps, xprime=configuration, asy=True)
    ...
    def predictive_old(self, conf_nr, steps, fro=None):
        ...
        """
        if fro is None:
            meansCur, std2sCur, divbyCur = self.ps.pred_old(
                conf_nr=conf_nr + 1, steps=steps)
        else:
            meansCur, std2sCur, divbyCur = self.ps.pred_old(
                conf_nr=conf_nr + 1, steps=steps, fro=fro)
...
class PredSamples(object):
    ...
    samples: ndarray(number_samples, D)
               All samples generated by the MCMC from LikIntegrate. Each sample contains sample 
    ...
    def __init__(self,
        ...
        if predHyper:
            self.ph = PredictiveHyper(
                x_train, y_train, x_test, invChol=self.invChol, samenoise=self.samenoise)
        else:
            self.ph = None

        if predOld:
            self.po = PredictiveOld(
                x_train, y_train, x_test, samenoise=self.samenoise)
        else:
            self.po = None

        if predNew:
            self.pn = PredictiveNew(
                x_train, y_train, x_test, samenoise=self.samenoise)
        else:
            self.pn = None

        self.C_samples = np.zeros(
            (samples.shape[0], self.x_train.shape[0], self.x_train.shape[0]))
        self.mu_samples = np.zeros(
            (samples.shape[0], self.x_train.shape[0], 1))
    ...
    def pred_new(self, steps=13, xprime=None, y=None, asy=False):
        ...
        if asy is False:
            asy_mean, std2star, _ = self.pred_hyper(xprime)
        else:
        ...
        for i in xrange(self.samples.shape[0]):
            ...
            mean_one, std2_one = self.pn.predict_new_point2(
                steps, asy_mean[1], y) #todo: correct so you need asy_mean[0]
...
class LikIntegrate(object):
    ...
    def create_configs(self, x_train, y_train, hyper_configs=40, chain_length=200, burnin_steps=200):
        ...
        sampler = emcee.EnsembleSampler(
            hyper_configs, fix + flex, pdl.marginal_likelihood)
        ...
        if not self.samenoise:
            if not self.horse:
                ...
                p0d = np.abs(hPrior.sample_from_prior(
...
class PredictiveHyper(object):
    ...
    def __init__(self,
        ...
        self.beta = beta
        self.theta0 = theta0
        if thetad is None or self.x.shape[-1] != thetad.shape[0]:
...
# change also here with the once and in all
    def inverse_chol(self, K):
        """
        One can use this function for calculating the inverse of K
...
#this version is more complete than version 2
    def calc_chol3(self, K):
        """
        Calculates the cholesky decomposition of the positive-definite matrix K
    ...
    def add_xtrain(self, xtrain):
        if self.x is None:
            ...
            self.x = np.append(self.x, xtrain, axis=0)

    def add_ytrain(self, ytrain):
        if self.y is None:
            self.y = ytrain
        else:
            self.y = np.append(self.y, ytrain, axis=0)

    def add_xtest(self, xtest):
        if self.xprime is None:
            self.xprime = xtest
        else:
            self.xprime = np.append(self.xprime, xtest, axis=0)
    ...
    def kernel_curve(self, t, tprime):
        ...
        cov = np.power(self.beta, self.alpha) / \
            np.power(((t[:, np.newaxis] + tprime) + self.beta), self.alpha)
...
#this version should not be used, because it's not protecting against bad situations
    def kernel_hyper(self, x, xprime):
        ...
        fiveR2 = 5 * r2
        cov = self.theta0 * (1 + np.sqrt(fiveR2) + (5 / 3.)
                             * fiveR2) * np.exp(-np.sqrt(fiveR2))
    ...
    def kernel_hyper2(self, x, xprime):
        ...
        try:
            r2 = np.sum(((x[:, np.newaxis] - xprime)**2) /
                        self.thetad**2, axis=-1)
            fiveR2 = 5 * r2
            result = self.theta0 * \
                (1 + np.sqrt(fiveR2) + (5 / 3.) *
                 fiveR2) * np.exp(-np.sqrt(fiveR2))
            result = result + \
                np.eye(N=result.shape[0], M=result.shape[1]) * self.noiseHyper
            return result
        except:
            return None
...
class PredictiveOld(object):
    ...
    def __init__(self,
        ...
        self.beta = beta
        self.theta0 = theta0
        # if theta_d is None or self.x.shape[-1] != len(theta_d):
    ...
    def calc_gamma(self, m_const):
        ...
        for yn in self.y:
            ...
            ktn_inv = self.inverse(chol_ktn)
            one_n = np.ones((ktn.shape[0], 1))
            gamma[index, 0] = np.dot(
                one_n.T, np.dot(ktn_inv, yn - m_const[index]))
            index += 1
