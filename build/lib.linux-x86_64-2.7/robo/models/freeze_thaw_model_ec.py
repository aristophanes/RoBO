...
"""
Compared to version 0 which is actually the standard, this economic version is
trying to implement a version of the method so that the training just considers
a certain number of points from the learning curve.

1.(x) We need a self.flag which says whether we should proceed economically:
   self.economically. And we need the maximal number of steps to be considered
   self.max_steps_train

2.(x) We need to identify where exactly we are using steps and the accuracies
   of the curve so that we could limit it to the lest k number of steps and
   accuracies, self.max_steps_train.
   - for the cases where only the length or the pure accuracies are necessary
     then you can just use the fct reduce().
   - for case where the steps are necessary maybe using something in functions
     kernel_curve() getKtn() would be a good decision. But better with function
     flags than class flags, because these functions are used in other contexts.
     The flag is standard false, and then it's put to true in case we need here a 
     economical version.

3.(x) Then implement it.

4. And test-reimplement-cycle until it works.
"""
...
class FreezeThawGP(BaseModel):
    ...
    def __init__(self,
        ...
        self.lg = lg
        self.economically = economically
        self.max_steps_train = max_steps_train
    ...
    def marginal_likelihood(self, theta):
        ...
        #!!!!!!!!!!!!!!!!!(x)
        x = self.x_train
        if not self.economically:
            y = self.y_train
        else:
            y = deepcopy(self.y_train)
            y = self.reduceY(y)
        ...
        if not self.economically:
            if self.lg:
                Lambda, gamma = self.lambdaGamma(self.m_const, covT=kt)
            else:
                Lambda, gamma = self.gammaLambda(self.m_const)
        else:
            if self.lg:
                Lambda, gamma = self.lambdaGamma(self.m_const, covT=kt, y_complete=y)
            else:
                Lambda, gamma = self.gammaLambda(self.m_const, ec=True)
    ...
    def reduceY(self, y):
        """
        This reductions is only important when the steps aren't be considered
        but just the length of the curve, or just the accuracies, provided
        the steps are already the right ones.
        """
        for i in xrange(len(y)):
            if len(y[i])>self.max_steps_train:
                y[i] = y[i][:-self.max_steps_train]
    ...
    def lambdaGamma(self, m_const, covT=None, y_complete=None):
        """
        ...
        for i, yn in enumerate(y_all):
            ...
            print 'one_n.T: ', one_n.T.shape
            print 'ktn_inv: ', ktn_inv.shape
            print 'yn: ', yn.shape
            print 'm_const: ', m_const.shape
            print
            gamma[i, 0] = np.dot(one_n.T, np.dot(ktn_inv, yn - m_const[i]))

        return Lambda, gamma

    def gammaLambda(self, m_const, ec=False):
        '''
        ...
        #if y_complete is None and ec is False:
        #    y_all = self.ys
        #else:
        #    y_all = y_complete
        ...
        for yn in y_all:
            ...
            t = np.arange(1, yn.shape[0]+1)
            if ec is True:
                if len(t) > self.max_steps_train:
                    t = t[:-self.max_steps_train]
            ...
            yn_now = yn
            if ec is True:
                if len(yn_now) > self.max_steps_train:
                    yn_now = yn_now[:-self.max_steps_train]

            gamma[index, 0] = np.dot(onenT_ktnInv, yn_now - m_const[index])            
            #if index == 0:
                #Lambda = ktn_inv
            #else:
    ...
    def getKt(self, y, ec=False):
        ...
        ktn = self.getKtn(y[0], ec=ec)
        O = block_diag(ktn)

        for i in xrange(1, y.shape[0]):
            # print 'in getKt() y[i]: ', y[i]
            ktn = self.getKtn(y[i], ec=ec)
