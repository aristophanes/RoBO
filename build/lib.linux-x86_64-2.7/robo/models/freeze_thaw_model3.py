...
"""
Trying to generate a comination of predNew with predOld
"""
...
class FreezeThawGP(BaseModel):
    ...
    def predict(self, xprime=None, option='asympt', conf_nr=0, from_step=None, further_steps=1, full_cov=False):
        """
        Predict using one of thre options: (1) predicion of the asymptote given a new configuration,
        ...
        full_cov: bool
            Whether full covariance has to delivered
    ...
    def pred_new1(self, step, asy_mean=None, y=None, mu_n=None, Cnn=None):
        yn = np.array([[1.]])

        t=np.arange(1,2).astype(float)
        tprime=np.arange(2,2+step+1)
    ...
    def pred_new_all1(self, conf_nr, steps, fro=None):
        """
        Here conf_nr is from 1 onwards. That's why we are using mu_n = mu[conf_nr - 1, 0] in the for-loop
        """
